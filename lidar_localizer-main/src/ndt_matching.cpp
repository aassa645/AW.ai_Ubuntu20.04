...
static void points_callback(const sensor_msgs::PointCloud2::ConstPtr& input)
{
  if (map_loaded == 1 && init_pos_set == 1)
  {
    ...
    if (_use_local_transform == true)
    {
      br.sendTransform(tf::StampedTransform(local_transform * transform, ros::Time::now(), "/map", "/base_link"));
    }
    else
    {
...
int main(int argc, char** argv)
{
  ...
  //estimate_twist_pub = nh.advertise<geometry_msgs::TwistStamped>("/current_velocity", 100);
