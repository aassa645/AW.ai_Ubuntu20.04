                                  ...
                                  nvinfer1::IHostMemory*& trt_model_stream)  // output buffer for the TensorRT model
{
  ...
  nvinfer1::IBuilderConfig* config = builder->createBuilderConfig(); //new
  ...
  config->setMaxWorkspaceSize(1 << 20); //new
  	
  

  //nvinfer1::ICudaEngine* engine = builder->buildCudaEngine(*network);
  nvinfer1::ICudaEngine* engine = builder->buildEngineWithConfig(*network, *config); //new
